...
| AIOps（开发预览） | 跟随当前 Tenant/Project、按 Cluster 切换工作区与会话、后台只读运行、固定 Cluster 概览取证、对话/轨迹 Tab、可重建上下文压缩、附件、归档/删除、导出与证据深链 |
...
- **开发预览：** AIOps 已提供跟随桌面 Tenant/Project、按 Cluster 隔离的会话，以及模型自主选择只读工具的 Agent
  循环、敏感工具审批、流式输出、可重建长上下文压缩与完整轨迹；资源变更、受控终端和部署仍在规划中。
