...
var serviceAuthorizedRoutes = map[string]string{
	...
	"GET /api/v1/ai/quota":                                              "ai.run is resolved for the selected cluster before project-scoped user quota is returned",
	"GET /api/v1/ai/evaluation":                                         "ai.run is resolved for the selected cluster and evaluation only includes the operator own turns",
	...
	"GET /api/v1/ai/sessions/:session_id":                               "session ownership and current fixed-cluster ai.run are resolved by the runtime",
	"PATCH /api/v1/ai/sessions/:session_id":                             "session ownership and current fixed-cluster ai.run are resolved before metadata changes",
	"DELETE /api/v1/ai/sessions/:session_id":                            "session ownership and current fixed-cluster ai.run are resolved before an archived session is deleted",
	"POST /api/v1/ai/sessions/:session_id/turns":                        "session ownership, fixed-cluster ai.run and every same-cluster evidence permission are rechecked by the runtime",
	"DELETE /api/v1/ai/sessions/:session_id/turns/current":              "session ownership and current fixed-cluster ai.run are resolved before cancellation",
	"POST /api/v1/ai/sessions/:session_id/approvals":                    "session ownership and current fixed-cluster ai.run are resolved before a parked tool call is answered",
	"GET /api/v1/ai/sessions/:session_id/trajectory":                    "session ownership and current fixed-cluster and evidence permissions are rechecked before bodies are returned",
	"GET /api/v1/ai/sessions/:session_id/context":                       "session ownership and the fixed cluster are rechecked before any context measurement is returned",
	"GET /api/v1/ai/sessions/:session_id/turns/:turn/feedback":          "session ownership and current fixed-cluster ai.run are rechecked before personal feedback is returned",
	"PUT /api/v1/ai/sessions/:session_id/turns/:turn/feedback":          "session ownership and current fixed-cluster ai.run are rechecked before personal feedback is saved",
	"GET /api/v1/ai/sessions/:session_id/events":                        "the same trajectory checks run on every poll and the browser session is reauthenticated throughout the stream",
	"GET /api/v1/ai/sessions/:session_id/attachments":                   "session ownership and current fixed-cluster ai.run are resolved before attachment metadata is returned",
	"POST /api/v1/ai/sessions/:session_id/attachments":                  "session ownership and current fixed-cluster ai.run are resolved before a bounded text attachment is stored",
	"DELETE /api/v1/ai/sessions/:session_id/attachments/:attachment_id": "session ownership and current fixed-cluster ai.run are resolved before an attachment is removed",
	"GET /api/v1/ai/sessions/:session_id/export":                        "session, trajectory evidence and attachments are reauthorized before export",
}
