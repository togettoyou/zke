...
export interface paths {
    ...
    "/api/v1/clusters/{cluster_id}/metrics-collector/jobs": {
         ...
         *     发现的 Service、最终 scheme/path/port/认证模式与就绪目标。
         *
         *     该调用会经 Agent 列出集群内 Service 与 EndpointSlice，因此只在进入详情页时
    ...
    "/api/v1/ai/tools": {
        ...
        /** @description AIOps 在本部署是否可用，以及运行时向模型公开的受控工具目录。描述的是运行时自身而不是任何 Cluster， 因此不需要作用域参数；每个工具在每次调用前仍会针对发起用户在会话固定 Cluster 上重新校验 permissions。 目录中的变更时间线额外要求 audit.read，并由审计服务按调用者可见范围过滤；变更后验证要求 cluster.read 与 cluster.event.read，只报告 Kubernetes 当前状态、Event 与收敛情况，业务指标验证继续走指标工具。 Cluster Terminal 命令工具在一个 Turn 的首次调用批准后重新计算并投射当前 Cluster 权限；后续命令复用同一 Terminal Pod 和冻结快照，Turn 结束或权限撤销时自动清理，且不投射 Secret 读写或 Agent Namespace 管理权限。 enabled 让 Console 在平台未启用 AIOps 时直接不展示该应用，而不必读取仅全局管理员可见的平台配置。 */
...
export interface components {
    schemas: {
        ...
        MetricsScrapeJob: {
            job_name: string;
            /**
             * @description 标注这个 Job 的开关写在哪个对象上。EndpointSlice 表示写在该 Service 的
             *     EndpointSlice 上——手工维护的 Endpoints 由 mirroring controller 把注解复制过去。
             * @enum {string}
             */
            source_kind: "Builtin" | "Service" | "EndpointSlice";
            namespace: string;
            source_name: string;
            /** @enum {string} */
            scheme: "http" | "https";
            ...
            /** @description 当前就绪的 EndpointSlice 地址；内置动态 Node Job 可为空。 */
            targets: string[];
