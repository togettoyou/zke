...
> 由模型自行判断的主动打开桌面应用，以及配额、诊断效果评估与用户反馈；
> 定时巡检与事件触发自动化仍在规划中。
...
何时压缩不是模型配置，而是部署策略：Server 配置文件里写的是**比例**（默认窗口的 80% 触发、最多保留窗口 16% 的
最近尾部），因此换一个上下文窗口更大的模型只需要改模型配置，不必再维护一个绝对的 token 阈值。运行时会先扣除输出
预留、系统提示词、工具 Schema 与检查点空间；剩余空间不足以原样容纳一个完整步骤时，该步骤也会进入结构化检查点。
检查点连同被替换的轨迹区间一起写入 `compaction`，连续压缩会合并上一份检查点与证据；端点若直接以"请求过大"拒绝，
也会压缩后重发一次。原始条目始终保持仅追加。

生成检查点期间，对话会显示“正在压缩上下文”以及本次待压缩的 token 数；这是运行阶段提示而不是可回放记录，不显示
无法可靠计算的百分比。压缩完成后，提示由包含压缩前后 token 数的正式检查点条目替换。
...
部署方可在 Server 的 `aiops.quota` 中设置 `daily_turns` 与 `daily_tokens`。两项都按“用户 + Project”统计并在
UTC 00:00 重置，`0` 表示不限制。Turn 配额在问题写入轨迹和调用模型之前原子检查；多个会话并发抢占最后一个
Turn 时只会有一个成功。Token 用量来自模型端点在 `model` 轨迹中报告的 input/output 用量，不把本地上下文估算
当作计量事实。

Token 限制是下一次 Turn 的准入线：已经开始的 Turn 允许有界完成，完成后若累计用量达到限制，后续 Turn 返回
HTTP 429。这样不会在一次诊断的结论前突然截断；单 Turn 仍受 Step、工具调用、上下文和总耗时预算保护。Console
在会话标题旁显示当日用量，配额耗尽后只禁用新提问，历史轨迹、反馈和效果评估仍可读取。

## 诊断效果评估与用户反馈

每个成功结束的 Turn 的结论下方都可以提交或修改反馈：回答是否有帮助、问题是否解决、结构化改进原因与可选备注。
反馈属于可修改的产品数据，单独存储，不回写 append-only 轨迹，也不会自动进入后续模型上下文。只能操作本人会话，
每次请求仍重新检查会话固定 Cluster 的 `ai.run`。

“效果”Tab 汇总当前用户在当前 Cluster 最近 30 天的可验证事实：Turn 完成/失败/取消数量、反馈覆盖率、帮助率、
解决率、平均工具调用和记录耗时，以及失败分类和结构化改进原因。帮助率与解决率只以已反馈 Turn 为分母；没有反馈
时显示无样本，而不是让模型评价自己的答案。接口最多允许查询 90 天，且不会聚合其他用户的会话。
