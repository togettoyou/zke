...
下钻视图的滚动内容末端由 `AppShell` 统一保留 16px 安全间距，确保长详情和命名空间配额表滚到底时最后一张卡片
不会贴住窗口边缘。容器服务的诊断导航 Provider 使用 `display: contents` 保持子视图直接参与 Shell 布局，不能用
固定高度包装层承接长详情，否则内容只会溢出包装层并覆盖 Shell 的底部间距。
...
降级，不把缺少证据解释为健康。
VPA 建议中的 CPU 保留 Kubernetes Quantity 写法（如 `109m`），内存与临时存储统一换算为易读的
KiB/MiB/GiB，避免直接展示 Kubernetes 规范化后的无单位字节整数。
...
默认化、准入修改与用户输入共同造成的真实增删行展示出来。普通文档使用精确行级差异，并按变更块只保留上下各
3 行未变化内容，相邻变更的上下文重叠时自动合并，中间以省略行标明折叠数量；Diff 区域有独立高度和滚动，不把确认
输入与按钮推出对话框。超大内嵌内容使用有界粗粒度差异、变化区间统计和抽样渲染，避免一个 4 MiB ConfigMap
卡住浏览器。Secret 仍只通过专用权限读取
...
着色而不是让编辑变卡。编辑器左侧显示行号，行号层只跟随纵向滚动，正文和着色层同时跟随横向与纵向滚动；行号
使用单个文本节点绘制，超过 20,000 行时关闭行号，避免异常大的嵌入文件产生数万个 DOM 节点。着色层跟随 textarea
的滚动位置，用的是 transform 位移而不是自身的
`scrollTop`/`scrollLeft`：只有 textarea 为滚动条留出布局空间，它因此比着色层矮一条水平滚动条、窄一条垂直
滚动条，也就能多滚这么多；把它的偏移量赋给滚动位置会被浏览器钳制在着色层自己较小的上限上，于是恰好在文档
滚到尽头时颜色落后字符一条滚动条的距离，位移则没有可被钳制的上限。横向与纵向滚动条同时出现时，其交汇角和
textarea resize 角使用当前主题的 `surface` 色，避免深色主题回退成浏览器默认白块。保存走两步，先服务端 DryRun，
再在确认弹窗中要求输入对象名称，并说明整份文档会替换现有对象、文档中
...
编辑器自带查找与替换。浏览器自己的 `Ctrl`/`⌘ + F` 看不见 textarea 里的内容，在这一个控件上按下去等于什么都
找不到，这比没有快捷键更糟——它读起来像「文本不在那里」。所以编辑器接管这个键：`Ctrl`/`⌘ + F` 打开查找，并把
文档中已选中的单行文本作为初始关键词；`Ctrl`/`⌘ + H` 打开替换（只读文档只打开查找，因为这个键不接管就会掀开
浏览器的历史记录侧栏）；`Enter` 与 `Shift + Enter`、`F3` 与 `Shift + F3`、`Ctrl`/`⌘ + G` 在匹配之间前后跳转；
`Esc` 关闭面板并把焦点交回文档。不用键盘也能用：编辑器右上角常驻一个查找图标按钮，面板里的上一个／下一个、
区分大小写、全词匹配、替换与全部替换都是带 `aria-label` 的图标按钮。

匹配只做纯文本，不支持正则表达式：这里的文档最大 4 MiB，一条回溯型正则在每次按键时重跑一遍不是「变慢」，
而是连同未保存的改动一起卡死标签页。匹配高亮画在语法着色层之下，与它共用同一份文本和同一套度量，因此高亮
落在哪些字符上由浏览器排版决定，不需要计算任何坐标；当前匹配用更重的橙色与其余匹配区分，两者都不复用选区的
蓝色——否则「即将被替换的那一处」和「浏览器选中的那一处」在同一屏上分不出来。一次最多标记 5,000 个匹配，
计数显示为 `5000+`；「全部替换」不受这个上限约束，替换到一半却看起来已经完成，是这里最坏的结果。全词匹配把
所有非 ASCII 字符都算作词内字符，否则一段中文里到处都是词边界，这个开关就没有意义了。
...
容器服务里的「Helm 应用」回答「这个命名空间里装了哪些应用、什么 Chart 版本、用的什么 values」——容器服务里
没有别的地方能回答它：资源对象浏览器看得到 Chart 生成的 Deployment，而 Deployment 上没有任何指回 Release 的
引用。它是**读**的那一半，另一半在同名的独立应用里，工具栏上的「Helm 应用」按钮就是通往它的入口。
...
Console 的容器高级设置以结构化行编辑端口，不开放 hostPort。「高级调度」同样不暴露接口 JSON，而是按 Kubernetes
本身的层级拆成节点亲和性、Pod 亲和性、Pod 反亲和性和拓扑分布四个表单页签：NodeAffinity 的硬性规则组之间明确标为
OR、组内标签与字段条件标为 AND；Pod 亲和与反亲和分别编辑 required/preferred、权重、拓扑键、Pod 与 Namespace
选择器、显式 Namespace 和动态标签键；拓扑分布逐条编辑偏差、拓扑键、不可满足策略、最少拓扑域与节点纳入策略。
创建和编辑都直接维护类型化结构，现有对象从详情无损回填，不存在需要用户学习的 ZKE JSON 方言。Console 在提交前检查
规则数量、键名、操作符和值组合、权重、重复项和字段联动，Server 继续执行同一组权威校验，最后仍由目标集群 DryRun
判定 schema、版本和准入策略。更新中省略这些字段表示兼容旧客户端并保留当前值；Console 始终显式提交，空对象或空数组
表示有意清除。该能力没有新增旁路接口，仍使用原工作负载创建/更新的权限、目标作用域、审计与并发前置条件。
