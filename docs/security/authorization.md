...
配额、反馈与效果评估不放大 `ai.run`：配额读取先用请求中的 Cluster 解析当前 Project 并判权，再只统计当前用户在该
Project 的用量；反馈读写先验证会话所有权和固定 Cluster，只接受已经成功结束的 Turn；效果评估只汇总当前用户在指定
Cluster 的运行与反馈。反馈与聚合不进入模型上下文，也不允许读取其他用户的会话。
