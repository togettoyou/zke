...
- [x] Pod Access（独立权限和 Agent Port Forward Stream、Pod UID/单端口绑定、WebSocket-first/SPDY fallback、
      Agent 回环桥接、双向字节/空闲/时长/并发限制、权限重验、无正文审计，以及同进程独立 HTTP/可选 HTTPS
      Listener、一次性激活地址、根路径 HTTP/SSE/WebSocket 代理、
...
- [x] Kubernetes Event（独立 `resource-watch.v1`、专用权限与 Agent 最小 RBAC、Namespace/资源过滤、
      ...
      尚未实现）
- [x] 资源用量（通过目标 Cluster Agent 读取 `metrics.k8s.io/v1beta1` NodeMetrics 与明确 Namespace 的
      PodMetrics，提供类型化 API、Agent 最小只读 RBAC，以及 Console 中对应 `kubectl top node/pod` 的
      30 秒刷新视图；Metrics Server 仍由集群自行安装，未安装或 APIService 暂不可用时返回明确的能力提示）
