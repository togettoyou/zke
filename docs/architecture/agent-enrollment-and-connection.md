...
Agent ConfigMap、ServiceAccount、最小 Role/RoleBinding、Node、Namespace、Pod、Pod Logs 与工作负载管理
...
该默认 ClusterRole 满足 Node、Namespace、Pod、五类工作负载和 Pod Logs 当前后端能力。其中 Pod Logs 只增加
`pods/log` 的 `get`，不授予 Exec 或 Eviction。Agent 的通用
