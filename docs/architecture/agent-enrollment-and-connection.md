...
Agent ConfigMap、ServiceAccount、最小 Role/RoleBinding、Node、Namespace、Pod、Pod Logs、Kubernetes Event、工作负载、服务路由、ConfigMap 与存储管理
...
该默认 ClusterRole 满足 Node、Namespace、Pod、五类工作负载、Service、Ingress、Gateway、ConfigMap、PV、PVC、StorageClass、Pod Logs 和 Kubernetes Event 当前后端能力。
...
在存储后端上线前使用旧清单接入的集群不会自动获得新增权限；管理员需要为同一 ClusterRole 增加
