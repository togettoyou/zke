...
用户登录进入 Console 桌面时，已保存的 Project 作用域和深链显式指定的 Project 优先恢复。两者都不存在时，Console
自动选择当前用户可见的第一个启用 Tenant，以及该 Tenant 下可见的第一个启用 Project；列表按名称排序。第一个
Tenant 下没有可见的启用 Project，或默认作用域查询失败时，保持未选择状态，由用户在顶部作用域选择器中明确选择。
