...
- 控制流从心跳请求/应答改为双向多路复用：当前 Agent 在两次心跳之间不读取控制流，Server 主动下推的
  GoAway 最长要等一个心跳周期才被处理，且任何非 HeartbeatAck 帧都会被判为无效应答而断连。下发指令、
  Web Terminal 和日志流之前必须先改为读写分离并按帧类型分发，同时重新划分断连原因分类；
