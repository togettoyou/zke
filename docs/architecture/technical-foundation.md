...
数据保留由 Server 的后台清理任务负责，配置见 `retention`。会话、Enrollment 及其尝试记录、被取代的 Agent
凭证都会在各自「结束」之后保留一段时间再删除——结束指会话过期或被吊销、Enrollment 被消费/吊销/失效、凭证被
吊销或过期，计时从结束时刻起算而非创建时刻。清理在启动时先执行一次，之后按 `sweep_interval` 周期执行；失败只
记录日志并等待下一轮，因为被清理的行本身已不可用。Agent 当前连接所使用的凭证永不被清理，即使它已被吊销且过期。

审计事件不在清理范围内：保留多久属于合规决策，没有可辩护的默认值，因此 `audit_events` 保留到运营方显式处理为止。
Pod 终端录像由自身的 `expires_at` 控制，在读写路径上顺带回收，不依赖上述任务。
...
- 默认集群业务权限包含 Node 的 `get`、`list`、`update`、`patch`，Namespace 的 `get`、`list`、`create`、`update`、`delete`，
  Pod 的 `get`、`list`、`update`、`delete`、`pods/log` 的 `get`、`pods/exec` 与 `pods/portforward` 的 `get/create`，以及 Deployment、StatefulSet、DaemonSet、Job 和
...
重试；Tenant、Project 或 Cluster 停用也会通知断连，但属于可恢复原因，恢复后 Agent 自动重连。Phase 2
Resource Stream 与 Kubernetes 资源闭环已经实现。
...
限制、响应正文全局内存预算和真实 QUIC 集成测试。Pod Logs 也已使用独立 `POD_LOGS` Stream 和
`pod-logs.v1` 能力实现有界快照与实时 Follow，支持取消、字节/并发/总时长上限和真实 QUIC 测试。生产 Agent
已通过 dynamic client 定域执行 Node 以及已获 Kubernetes RBAC 授权的
...
- Agent 默认 ClusterRole 为 Service、Ingress 与 Gateway 主资源增加完整 CRUD，为 ConfigMap、PV、PVC、
  ...
  `get/create`、为 Pod 端口转发增加 `pods/portforward` 的 `get/create`；三者各自由专用协议和权限定域；为 Node Drain
