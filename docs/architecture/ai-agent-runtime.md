...
> 状态：事件溯源会话、后台运行、SSE 续传与流式增量、可重建的摘要压缩、模型自主工具循环（只读工具目录）
> 与敏感工具审批等待已经实现；写工具、并行子任务与自动化仍在规划中。
...
工具目录由 `aitools` 组装，全部复用 ZKE 已有的读取服务，因此天然经过 Agent 传输与各自的响应上限。运行时本身没有
kubeconfig，也不直连 Kubernetes API Server。工具失败不会伪造成空集群，而是形成明确的失败结果，模型据此调整下一步。
...
敏感工具在 `ask` 与 `assisted` 模式下停下来等待用户批准，`full` 模式不停。三档都不扩大权限，改变的只是一段来自
Pod 日志的提示注入能走多远，因此每次请求都记录当时的模式。
