...
`retain_ratio` 是保留尾部的上限目标，不是无条件占用：运行时会先扣除单次最大输出、系统提示词、工具 Schema 和检查点
所需空间。当较小的上下文窗口无法同时容纳这些固定开销与一个完整的最近步骤时，该步骤也会进入检查点，避免已经达到
压缩线却仍以“上下文预算不足”结束。连续压缩会把上一份检查点和证据合并进新检查点，不会只保留最近一段轨迹。
...
- `delta` 是模型正在产生的正文，没有 id、不重放、不进数据库。它存在的意义只是让操作者看见答案在成形，对应
  步骤的 `model` 条目才是记录、导出和事后复核的依据。上下文压缩开始与结束也通过无 id 的瞬态状态发送，界面只显示
  “正在压缩上下文”及待压缩 token 数；完成后由正式 `compaction` 条目替换，不展示或持久化摘要生成过程。
