...
> 敏感工具审批等待、受控 Cluster Terminal 命令、随 Server 发布的排查技能与只读并行子任务已经实现；
> 定时巡检与事件触发自动化仍在规划中。
...
目录里有两个工具由运行时自己实现而不是转交 `aitools`，因为它们要的正是目录刻意没有的东西：`load_skill` 返回的是
Server 自己发布的文本，`run_subtasks` 启动的是带独立预算、审批与轨迹条目的模型对话。两者都只声明 `ai.run`——这
不是让 `ai.run` 顶替某个集群权限，而是它们本身不触达集群；技能推荐的每一步、分支执行的每一次读取仍各自独立授权。
